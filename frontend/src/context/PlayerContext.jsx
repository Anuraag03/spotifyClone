...
import { PlayerContext } from "./PlayerContextInstance";
...
const PlayerContextProvider = (props) => {
  ...
  const seekSong = async (e) => {
    ...
    seekBg.current.style.setProperty('--seek-width', `${(e.nativeEvent.offsetX / seekBg.current.clientWidth) * 100}%`);
    setTime({
      currentTime: {
  ...
  useEffect(() => {
    setTimeout(()=>{
      audioRef.current.ontimeupdate=()=>{
        ...
        seekBg.current.style.setProperty('--seek-width', `${(audioRef.current.currentTime / audioRef.current.duration) * 100}%`);
        setTime({
          currentTime:{
            second:Math.floor(audioRef.current.currentTime % 60),
            minute:Math.floor(audioRef.current.currentTime / 60)
